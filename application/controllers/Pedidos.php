...
class Pedidos extends REST_Controller {
    ...
    public function realizar_orden_post($token = "0", $id_usuario = "0"){
        ...
        // verifica si existe en la data los items || el largo del strign es igual a 0
        ...
        // compara el id y el token que le esta mandando la aplicacion
        ...
        $this->db->reset_query(); // borrar datos en los querys usados

        $insertar = array('usuario_id' => $id_usuario );
        $this->db->insert( 'ordenes', $insertar ); // insertar en la tabla ordenes el id del usuario
        $orden_id = $this->db->insert_id(); //obtener orden_id de la ultima inserción

        // Crear el detalle de la orden
        $this->db->reset_query();
        $items = explode( ',', $data['items'] ); // separar por coma
